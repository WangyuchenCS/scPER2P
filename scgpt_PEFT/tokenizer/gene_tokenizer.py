...
def tokenize_batch(
    ...
    for i in range(len(data)):
        ...
        if include_zero_gene:
            ...
            idx = np.nonzero(row)[0]  # 一行基因 (2808,) 中非零元素的 索引! (251,) (1037,),(199,)...不规则
            values = row[idx]  # 一行基因 (2808,) 中非零元素的 值! (251,) (1037,),(199,)...不规则
            genes = gene_ids[idx]  # 一行基因 (2808,) 中非零元素的 哪些基因! (251,) (1037,),(199,)...不规则
            if mod_type is not None:
                mod_types = mod_type[idx]
        if append_cls:  # True  # append <cls> token at the beginning as cell representation
            genes = np.insert(genes, 0, cls_id)  # 在 genes 数组的开头（索引为 0 的位置）插入元素或数组cls_id。如果 cls_id 是单个元素，它将被直接插入到数组的开头；如果 cls_id 是一个数组或列表，那么它的元素将按顺序被插入到 genes 数组的开头。
            values = np.insert(values, 0, 0)  # 在 values 数组的开头（索引为 0 的位置）插入元素或数组0。
            if mod_type is not None:
...
def pad_batch(
    ...
    """
    max_ori_len = max(len(batch[i][0]) for i in range(len(batch)))  # 431  1401
...
#     train_data,  # (7844, 2808)
#     gene_ids,  # 2808
#     max_len=max_seq_len,  # 2001
#     vocab=vocab,
#     pad_token=pad_token,  # '<pad>'
#     pad_value=pad_value,  # -2
#     append_cls=True,  # append <cls> token at the beginning
#     include_zero_gene=include_zero_gene,  # False
# )
def tokenize_and_pad_batch(
    ...
    tokenized_data = tokenize_batch(
        data,
        gene_ids,
        return_pt=return_pt,  # True
        append_cls=append_cls,  # True
        include_zero_gene=include_zero_gene,  # False
        cls_id=cls_id,  # 60695
        mod_type=mod_type,  # None
        cls_id_mod_type=cls_id_mod_type if mod_type is not None else None,  # None
    )
...
def random_mask_value(
    ...
    for i in range(len(values)):
        row = values[i]
        non_padding_idx = np.nonzero(row - pad_value)[0]  # pad_value=-2,为了不选取pad_value 将数值加成0,然后选非0来避免选到pad_value。
        n_mask = int(len(non_padding_idx) * mask_ratio)
        mask_idx = np.random.choice(non_padding_idx, n_mask, replace=False)  # 从非0的索引non_padding_idx中随机选取n_mask个
...
def mask_protein(
    values: Union[torch.Tensor, np.ndarray],
    mod_types: Union[torch.Tensor, np.ndarray],
    mask_ratio: float = 0.15,
    mask_value: int = -1,
    pad_value: int = 0,
) -> torch.Tensor:
    ...
    if isinstance(mod_types, torch.Tensor):
        mod_types = mod_types.clone().detach().numpy()
    else:
        mod_types = mod_types.copy()


    for i in range(len(values)):
        row = values[i]
        # mask_idx = mod_types
        mask_idx = (mod_types[i] == 0)  # Protein mod_type is 0
        row[mask_idx] = mask_value  # mask_value = -1
    return torch.from_numpy(values).float()