...
from scgpt_PEFT.tokenizer import tokenize_and_pad_batch, random_mask_value, mask_protein
from scgpt_PEFT import SubsetsBatchSampler
from scgpt_PEFT.loss import (
    masked_relative_error,
    criterion_neg_log_bernoulli,
)
from scgpt_PEFT.utils import eval_scib_metrics
import warnings
...
def prepare_data(
    ...
    masked_values_train = mask_protein(
        tokenized_train["values"],
        tokenized_train["mod_types"],
        mask_ratio=config.mask_ratio,
        mask_value=config.mask_value,
        pad_value=config.pad_value,
    )
    masked_values_valid = mask_protein(
        tokenized_valid["values"],
        tokenized_valid["mod_types"],
        mask_ratio=config.mask_ratio,
        mask_value=config.mask_value,
        pad_value=config.pad_value,
    )
    ...
    print(
        f"Protein masking at epoch {epoch:3d}, with protein", f"{(masked_values_train[0] == config.mask_value).sum()}",
        "and with ratio of masked values in train: ",
        f"{(masked_values_train == config.mask_value).sum() / (masked_values_train - config.pad_value).count_nonzero():.4f}",
    )
...
def evaluate(
    ...
    return (total_loss / total_num), output_dict_storage, batch_data_storage
