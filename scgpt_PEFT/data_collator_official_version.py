from dataclasses import dataclass  # from dataclasses import dataclass：这导入了 Python 的 dataclass 装饰器，它用于自动生成特殊方法，如 __init__() 和 __repr__()，简化类的定义。dataclass 适用于主要用于存储数据的类。
...
@dataclass  # 这是一个装饰器，用于装饰紧随其后的类 DataCollator。
# 使用 @dataclass 装饰器可以自动为类生成初始化方法 (__init__)、表示方法 (__repr__) 和比较方法 (__eq__ 等)，根据类属性简化类的定义。
# 它是 Python 3.7+ 新增的功能，旨在减少样板代码。
class DataCollator:
    """
