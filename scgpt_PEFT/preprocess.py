...
class Preprocessor:
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        is_logged = self.check_logged(adata, obs_key=key_to_process)  #  Check if the data is already log1p transformed.  # 最大不能超过30,最小不能小于0 非0最小值不能>=1
        ...
        if (isinstance(self.filter_cell_by_counts, int) and self.filter_cell_by_counts > 0):
            ...
            sc.pp.filter_cells(adata, min_counts=self.filter_cell_by_counts if isinstance(self.filter_cell_by_counts, int) else None,)

        # step 3: normalize total  # # TODO: begain here!!!!
        if self.normalize_total:
            logger.info("Normalizing total counts ...")   # adata 21312 × 2808
            normed_ = sc.pp.normalize_total(adata, target_sum=self.normalize_total if isinstance(self.normalize_total, float) else None,  # 这里是10000，意味着每个细胞的表达量将被缩放，使得它们的总和达到10000。
                layer=key_to_process,
                inplace=False,
            )["X"]
            key_to_process = self.result_normed_key or key_to_process
            _set_obs_rep(adata, normed_, layer=key_to_process)  # 增加adata.layers['X_normed'] = normed_
        ...
        if self.binning:
            ...
            for row in layer_data:
                if row.max() == 0:
                    ...
                    continue
                non_zero_ids = row.nonzero()  # 数组中的每个数字代表row数组中非零元素的位置索引，输出(array([0, 11, 14, ..., 2799]),)意味着row数组中位置0、11、14等的元素是非零的
                non_zero_row = row[non_zero_ids]
                bins = np.quantile(non_zero_row, np.linspace(0, 1, n_bins - 1))  # 50个结果
                # bins = np.sort(np.unique(bins))
                # NOTE: comment this line for now, since this will make the each category
                # has different relative meaning across datasets
                non_zero_digits = _digitize(non_zero_row, bins) # 将non_zero_row中的元素分别映射到bins中的位置，返回的是一个数组
                assert non_zero_digits.min() >= 1
                assert non_zero_digits.max() <= n_bins - 1
                binned_row = np.zeros_like(row, dtype=np.int64)  # 生成一个和row数组一样大小的数组，但是元素都是0
                binned_row[non_zero_ids] = non_zero_digits # 将non_zero_digits中的bin值赋给binned_row中的对应位置
                binned_rows.append(binned_row) # 将binned_row加入到binned_rows中
                bin_edges.append(np.concatenate([[0], bins])) # 将0和bins拼接在一起 为什么要加0呢？ # 为了保证每个bin的最小值都是0
            adata.layers[self.result_binned_key] = np.stack(binned_rows)  # 将binned_rows加入到adata.layers中，存储离散化的数据 (21312, 2808)
            adata.obsm["bin_edges"] = np.stack(bin_edges) # 将bin_edges加入到adata.obsm中，存储箱子的边界 (21312, 51)

    def check_logged(self, adata: AnnData, obs_key: Optional[str] = None) -> bool:
        """
        Check if the data is already log1p transformed.  # 最大不能超过30,最小不能小于0 非0最小值不能>=1
